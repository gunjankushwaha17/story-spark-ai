...
=======
>>>>>>> ba355ec (feat/plot-hole-analyzer)
...
>>>>>>> ba355ec (feat/plot-hole-analyzer)
  ...
  // Memoised sentence segments for audio narration highlighting
  ...
  const handleExportDOCX = async () => {
    ...
    try {
      ...
      console.error(error); toast.dismiss(toastId); toast.error("Failed to export Word document.");
    }
  };
  ...
  const handleExportTXT = () => {
    ...
    } catch (error) { console.error(error); toast.error("Failed to export text file."); }
  };
...
<navMap><navPoint id="navpoint-1" playOrder="1"><navLabel><text>${title}</text></navLabel>
<content src="chapter1.xhtml"/></navPoint></navMap></ncx>`;
      ...
      console.error(error); toast.dismiss(toastId); toast.error("Failed to export EPUB.");
    }
  };
